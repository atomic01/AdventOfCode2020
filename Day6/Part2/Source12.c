...
int main()
{
	...
	while (i < 1006) // to get past the text for the puzzle in the txt file where i stored the numbers
	{
		fscanf(fp, "%c", &buffer);
		//printf("%c", buffer);
		i++;
...
{
	int count = 0, numberOfVoters = 0, i = 0;
	char alphabet[26] = "abcdefghijklmnopqrstuvwxyz";
	int letterCounts[26] = {0};


	while(votingData[i] != '\0')
	{
		if(votingData[i] == '#')
			numberOfVoters++;
		i++;
	...
	for(int i = 0; i < 26; i++)
		if(letterCounts[i] == numberOfVoters)
			count++;
